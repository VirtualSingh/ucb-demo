...
@Component({
  selector: 'app-pqc-form',
  templateUrl: './pqc-form.component.html',
  styleUrls: ['./pqc-form.component.scss'],
})
export class NewPQCFormComponent implements OnInit {
  ...
  onDrugsChange(e: any) {
    const drugsArrayTemp: FormArray = this.pqcForm.get('drugs') as FormArray;
    const medArray: FormArray = this.pqcForm.get('medicationArray') as FormArray;
  ...
  get drugs() {
    return this.pqcForm.get('drugs') as FormArray;
  }
  ...
  onSubmit() {
    // console.log(this.pqcForm.value);

    // let estimated_dob = this.pqcForm.value.patientDetails?.estimated_dob;
    // this.surveyJson.patient.estimatedBirthDate = new DatePipe('en').transform(estimated_dob, 'd-MM-yyyy') || '';

    // let menstrual_date = this.pqcForm.value.patientDetails?.menstrual_date;
    // this.surveyJson.patient.firstDayLMP = new DatePipe('en').transform(menstrual_date, 'd-MM-yyyy') || '';

    // let patient_dob = this.pqcForm.value.patientDetails?.patient_dob;
    // this.surveyJson.patient.patientDateOfBirth = new DatePipe('en').transform(patient_dob, 'd-MM-yyyy') || '' || '';

    // this.surveyJson.patient.patientInitials = this.pqcForm.value.patientDetails?.patient_initials || '';

    // this.surveyJson.product.medicationTakenByPatient = this.pqcForm.value.drugs || [];
    // let j=0;
    // this.pqcForm.value.medicationArray?.forEach((regimenItem: any, i) => {
    ...
    // this.surveyJson.medicalHistory.patientMedicalHistory = this.pqcForm.value.medicalHistory?.patientMedicalHistory || '';

    // this.surveyJson.complications.previousPregnancyComplications[0] = this.pqcForm.value.complications?.previousPregnancyComplications || '';

    // let pregnancyOutcomeDate = this.pqcForm.value.complications?.pregnancyOutcomeDate || '';
    // this.surveyJson.complications.pregnancyOutcomeDate = new DatePipe('en').transform(pregnancyOutcomeDate, 'd-MM-yyyy') || '';
    // this.surveyJson.complications.pregnancyOutcome[0] = this.pqcForm.value.complications?.pregnancyOutcome || '';
    // this.surveyJson.complications.newBornGender = this.pqcForm.value.complications?.newBornGender || '';
    // this.surveyJson.complications.height = parseInt(this.pqcForm.value.complications?.height || '') || 0 ;
    // this.surveyJson.complications.heightUnits = this.pqcForm.value.complications?.heightUnits|| '';
    // this.surveyJson.complications.weight = parseInt(this.pqcForm.value.complications?.weight || '') || 0;
    // this.surveyJson.complications.weightUnits = this.pqcForm.value.complications?.weightUnits || '';
    // this.surveyJson.complications.apgarScore = this.pqcForm.value.complications?.apgarScore || '';
    // this.surveyJson.complications.newBornSufferedCongInfo = this.pqcForm.value.complications?.newBornSufferedCongInfo || '';
    // this.surveyJson.complications.riskFactorsForReportedMalformations = this.pqcForm.value.complications?.riskFactorsForReportedMalformations || '';
    // this.surveyJson.complications.congMalfRelatedToMedications = this.pqcForm.value.complications?.congMalfRelatedToMedications || '';
